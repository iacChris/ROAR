...
Examples:
  ...
  depth_array2 = RgbImageToFloatArray(depth_rgb)
  ...
  depth_array3 = RgbImageToFloatArray(depth_gray)
...
def ClipFloatValues(float_array, min_value, max_value):
  ...
  if float_array.min() < min_value or float_array.max() > max_value:
    print('Image has out-of-range values [%f,%f] not in [%d,%d]' % (float_array.min(), float_array.max(), min_value, max_value))
...
def FloatArrayToRgbImage(float_array,
  ...
  Returns:
    24-bit RGB numpy array representing depth values.
  """
  ...
  # Calculate:
  #   b = (f / 256) / 256  high byte
  #   g = (f / 256) % 256  middle byte
  #   r = f % 256          low byte
  bg = np.divide(int_array, 256)
  b = np.divide(bg, 256)
  g = np.mod(bg, 256)
  ...
  if not drop_blue:
    ...
    r = np.mod(int_array, 256)
    rgb_array[..., 0] = r
  #image_mode = 'RGB'
  #image = Image.fromarray(rgb_array, mode=image_mode)
  return rgb_array

def RgbImageToFloatArray(image, scale_factor=None):
  ...
  if channels == 3:
    ...
    float_array = np.sum(image_array * [1, 256, 65536], axis=2)
